...
[[ "${@#*--debug}" != "${@}" ]] && DEBUG=${true}
declare -i LOG_DEBUG=${true}

declare -i NO_TTY="${false}"
...
declare PIC_FILE="preInstallCheck.sh"
declare PRE_INSTALL_CONF_DIR="/etc/cloudian"
declare PRE_INSTALL_CONFIG="PreInstallConfig.txt"
declare CENTOS_VERSION="$(grep -oE '[0-9]' /etc/redhat-release | head -1)"
declare SELFEXTRACT="selfextract_prereq_el${CENTOS_VERSION}.bin"
...
function __getCursorPosition() {
	...
	# read from /dev/tty to avoid pulling input redirection text
	read -sdR -p $'\E[6n' _position < /dev/tty
...
function __getNewVLANConfigFileName() {
	...
	: ${2:?$(__raiseError 'VLAN is required')}
	local -i _vlan="$(( 10#$2 ))"
	...
	while read _interface _status _details; do
		...
		if [[ -n "${_runningIPAddress:-}" ]]; then
			...
			for _runningIPAddress in $(ip --oneline address show ${_interface:-} | awk 'match($3, /^inet6?$/) {print $4}'); do
				local _ipOutputFormat="${_outputFormat}"
				echo "${_runningIPAddress}" | grep -q ':' && _ipOutputFormat='++%s\t  %s\t%s\t%s\t%s\t%s\t%s\n'
				printf "${_ipOutputFormat}" "${_interface:-}" "${_runningIPAddress:---}" "${_linkState:-}" "${_linkType:-}" "${_linkMode:-}" "${_linkMaster:-}" "${_linkSpeed:-}"
				_interface=' '
...
function __restartNetworkInterface() {
	...
	#remove resolv.conf.save if it exists
	ss__deleteResolvConfSaveFile
    ...
    __printMessage "Restarting ${_interface} ... " ${false}; _cursorPosition=($(__getCursorPosition)); __printMessage
    ip addr flush dev "${_interface}"
    if ifdown "${_interface}" && ifup "${_interface}" > /dev/null 2>&1; then
        __printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done\n" ${true}
    else
        __printStatusMessage ${_cursorPosition[*]} "${IRED}Failed (${?})\n"
		__printMessage "${IRED}The IP address $(__printInterfaceConfigValue "${_interface}" 'IPADDR') may already be in use. Please change the configured IP address for interface ${_interface}!" "${true}"
        return 1
    fi
...
declare ss_uri='http://hyperstore-releases.s3-website.cloudianhyperstore.com/Scripts/system_setup.sh'
declare ss_uri_beta='http://hyperstore-releases.s3-website.cloudianhyperstore.com/Scripts/system_setup2.sh'
declare ss_versionsURL='http://hyperstore-releases.s3-website.cloudianhyperstore.com/versions.txt'

declare -a RESTRICTED_PASSWORDS=('password' 'assword' 'cloudian' 'drowssap' 'p@ssw0rd') #Restricted password options
declare -a REQUIRED_PACKAGES=('bc' 'bind-utils' 'dmidecode' 'facter' 'gdisk' 'nmap-ncat' 'ntp' 'ntpdate' 'openssh' 'pax' 'pciutils' 'sshpass' 'unzip' 'wget')
...
declare -A ss_mountOptions=(
	['ext4']='defaults,rw,nosuid,noexec,nodev,noatime,noinit_itable,data=ordered,errors=remount-ro'
	['xfs']='defaults,rw,nosuid,noexec,nodev,noatime'
)
declare ss_LAZYINIT_MOUNTNOINIT=1
	...
	if [[ "$(ss__getSSHKeyFileName)" != "${_input}" ]]; then
		if [[ -f "$(ss__getSSHKeyFileName)" && -d "$(dirname "${_input}")" ]]; then
			if __getYesNoInput 'Move your existing SSH key file to the new location?'; then
				__printMessage "\n${IWHITE}Moving SSH key file ... " ${false}; _cursorPosition=($(__getCursorPosition)); __printMessage
				if mv "$(ss__getSSHKeyFileName)" "${_input}" 2>&1 | tee -a ${BASH_SOURCE[0]}.log; then
					__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done\n" ${true}
				else
					__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed (${?})\n"
					__pause
...
function ss__cleanupInstallation() {
	...
	if __getYesNoInput 'Are you sure you want to do this?' "${IRED}No"; then
		...
		_sshCommand=$(cat <<-EOF
				...
				for _file in ${ss_STAGING_DIRECTORY}/CloudianHyperStore-*.bin* ${ss_STAGING_DIRECTORY}/*.lic ${ss_STAGING_DIRECTORY}/cloudian-docs*.tar.gz* ${ss_STAGING_DIRECTORY}/system_setup*.sh ${ss_STAGING_DIRECTORY}/${PIC_FILE} ${ss_STAGING_DIRECTORY}/*.csv ${ss_STAGING_DIRECTORY}/*.pdf ${ss_STAGING_DIRECTORY}/cloudian-installation-key*; do
...
function ss__configureDisk() {
	...
	ss__formatDisk "${_disk}1" "${_filesystemType}" "${ss_LAZYINIT_MOUNTNOINIT}" >/dev/null || {
		_returnCode=${?}
		...
		if __createDirectory "${ss_DISK_MOUNTPATH}${_mountpoint}" >/dev/null 2>&1; then
			...
			if flock --exclusive --nonblock ${_fd}; then
				...
				(
					...
					local __mountOptions="${ss_mountOptions[${_filesystemType,,}]}"
					[[ "${ss_LAZYINIT_MOUNTNOINIT}" -eq 0 ]] && __mountOptions="${__mountOptions//',noinit_itable'/}"
					printf 'UUID=%s\t%s\t%s\t%s\t0\t1\n' "${_uuid}" "${ss_DISK_MOUNTPATH}${_mountpoint}" "${_filesystemType,,}" "${__mountOptions}" >> ${FSTAB}

					flock --unlock ${_fd2}
					__logMessage "Unlocked ${FSTAB} for: '${_disk}'"
					if [[ ${CENTOS_VERSION} -eq "7" ]] && ! systemctl daemon-reload; then
						__logMessage "systemd daemon-reload failed after adding ${ss_DISK_MOUNTPATH}${_mountpoint} to ${FSTAB}"
					fi
				)
				exec {_fd2}>&-
...
function ss__createConfigFile() {
	...
	if [ ! -f "${_config_dir}/${_config_file}" ]; then
		touch "${_config_dir}/${_config_file}"
	fi
...
function ss__configureHyperStorePrerequisites() {
	...
	if [[ ${CENTOS_VERSION} -eq 7 ]]; then
		__printMessage "${IWHITE}Checking firewalld Status ... " ${false}; _cursorPosition=($(__getCursorPosition)); __printMessage
		if systemctl status firewalld 1>/dev/null; then
			__printStatusMessage ${_cursorPosition[*]} "${IRED}Enabled"
			__printMessage
			__printMessage 'HyperStore requires that firewalld is disabled.'
			if (( ${_assumeYes} )) || __getYesNoInput 'Would you like to disable it now?' 'Yes'; then
				__printMessage '\tDisabling firewalld ... ' ${false}
				if systemctl stop firewalld >/dev/null 2>&1 && systemctl disable firewalld; then
					__printMessage "${IGREEN}Done"
				else
					__printMessage "${IRED}Failed"
				fi
...
#ss__copyFileToCluster(File, ClusterPassword='', singleNode='')
...
function ss__copyFileToCluster() {
	...
	ss__runOnCluster "mkdir -p '$(__getDirectoryName "${_file})")'" "${_clusterPassword:-}" ${true} "${_singleNode}"

	__printMessage
	local -a _nodeList=($(ss__getSurveyFileEntries 'IP' 2>/dev/null))
	! [[ -z "${_singleNode}" ]] && _nodeList=("${_singleNode}")
	for _server in "${_nodeList[@]}"; do
...
function ss__createNetworkConfig() {
	...
	[[ "${_type,,}" == 'vlan' ]] && {
		_interface="$(echo ${_interface} | sed "s/${_interfaceMaster}.//g" | sed 's/[^0-9]*//g')"
		...
		'bond'|'ethernet'|'vlan')
			...
			case "${_addressType,,}" in
				...
				'static') #Static IP Configuration
					...
					while ping -c 1 -W 3 "${_ipAddress}" > /dev/null 2>&1 && [[ "${_ipAddress:-}" != ${_configuredIPAddress} ]] && [[ "${_ipAddress:-}" != '0.0.0.0' ]]; do
						__printMessage "The IP address '${_ipAddress}' is already in use, please choose a different one!\n"
						__getIPAddressInput 'IP Address:' '_ipAddress' "${_currentIPAddress:-${_configuredIPAddress:-0.0.0.0}}" || return ${?} && _cursorPosition=($(__getCursorPosition));
					done
			...
			case "${_type,,}" in
				'bond')
					...
					printf 'BONDING_OPTS="mode=%s miimon=100"\n' "${_bondingType}" >> "${_interfaceConfigFile}.new"
	...
	if [[ -n "${_ipAddress:-}" ]]; then
		...
		if ! grep -i -q "${_ipAddress}" "${HOSTS_FILE}" && [[ -n "${_currentHostname}:-" ]] && __getYesNoInput "Will ${IWHITE}${_ipAddress}${RST} be the address you use for ${IWHITE}${_currentHostname}${RST} in your survey file?" "${IRED}No"; then
			...
			if [[ -n "${_currentHostname:-}" && "${_currentHostname,,}" != 'localhost' && "${_currentHostname,,}" != 'localhost.localdomain' ]]; then
...
#ss__deleteResolvConfSaveFile()
	#Deletes /etc/resolv.conf.save if it exists
	#Return Codes:	0=Success
function ss__deleteResolvConfSaveFile() {
	trace "${FUNCNAME[0]}" "$@"

	[[ ${#} -ne 0 ]] && __raiseWrongParametersError ${#} 0

	if [[ -f "${RESOLV_CONF}.save" ]]; then
		__removeFile "${RESOLV_CONF}.save"
	fi
...
function ss__extractPackagedFile() {
	...
	[[ ! -f "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" && ! -f "${_hyperstoreBinaryFile}" ]] && return 1
	__createDirectory "${_targetDirectory}" >/dev/null || return 4
	if [[ ! -f "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" ]]; then
		ss__extractHyperStoreBinaryPackagedFile "${SELFEXTRACT}" "${ss_STAGING_DIRECTORY}" "${_hyperstoreBinaryFile}" || return ${?}
	fi
	tail -n+$(awk '/^__ARCHIVE_BELOW__/ {print NR + 1; exit 0; }' "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}") "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" | tar -xOz "./rpm_el${CENTOS_VERSION}.tar" | tar --wildcards -x -C "${_targetDirectory}" "${_rpm}*" 2>/dev/null || return 3

	return 0
}

#ss__formatDisk(Disk, FileSystem='ext4', enable_lazy_init=1)
...
function ss__formatDisk() {
	...
	case "${_fileSystem,,}" in
		'ext4')
			if ! mkfs.ext4 -q -i 8192 -m 0 -E lazy_itable_init="${_enable_lazy_init}",discard -O dir_index,extent,flex_bg,large_file,sparse_super,uninit_bg "${_disk}" >/dev/null 2>&1; then
				__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
				return 1
			else
...
function ss__installBundledPackages() {
	...
	local _selfExtractBundle="${ss_STAGING_DIRECTORY}/${SELFEXTRACT}"
	local _tempDirectory="$(mktemp --directory --tmpdir)"

	if [[ ! -f "${_selfExtractBundle}" && -f "${_hyperstoreBinaryFile}" ]]; then
		tail -n+$(awk '/^__ARCHIVE_BELOW__/ {print NR + 1; exit 0; }' "${_hyperstoreBinaryFile}") "${_hyperstoreBinaryFile}" | tar -xOz ./cloudian-*.tar.gz | tar -xz -C "${ss_STAGING_DIRECTORY}" ${SELFEXTRACT} || return 1
	fi
	...
	if [[ -f "${_selfExtractBundle}" ]]; then
		...
		__printErrorMessage "Unable to locate ${SELFEXTRACT} or Cloudian HyperStore Binary File in ${ss_STAGING_DIRECTORY}."
		return 1
	fi
...
#ss__installSSHKeyFile(SSHKeyFileFullPath, ClusterPassword="", singleNode="")
...
function ss__installSSHKeyFile() {
	...
	ss__runOnCluster "${_sshCommand}" "${_clusterPassword:-}" ${true} "${_singleNode}" || return ${?}
...
function ss__parseArguments() {
	...
	[[ "${_arguments}" != "${_arguments// --no-tty }" ]] && {
		__printDebugMessage "Enabling non-interactive mode"
		_arguments="${_arguments// --no-tty}"
		NO_TTY="${true}"
	}
	...
	[[ "${_arguments}" != "${_arguments#* --add-entry }" ]] && {
		if [[ "${NO_TTY}" -ne 0 ]]; then
			...
			if [[ -n "${_arguments// /}" ]]; then
				...
				local _region=$(echo "${_arguments}" | awk -F',' '{print $1}')
				local _hostname=$(echo "${_arguments}" | awk -F',' '{print $2}')
				local _ipAddress=$(echo "${_arguments}" | awk -F',' '{print $3}')
				local _datacenter=$(echo "${_arguments}" | awk -F',' '{print $4}')
				local _rack=$(echo "${_arguments}" | awk -F',' '{print $5}')
				local _interface=$(echo "${_arguments}" | awk -F',' '{print $6}')
				...
				ss__addSurveyFileEntry "$(ss__getSurveyFileName)" "${_region,,}" "${_hostname}" "${_ipAddress}" "${_datacenter}" "${_rack}" "${_interface}"

			else
				__printErrorMessage 'Please enter a survey file entry to add' ${true} 1
			fi
	...
	[[ "${_arguments}" != "${_arguments#* --configure-disks }" ]] && {
		...
		if [[ -n "${_arguments// /}" ]]; then
			...
			if [[ "${NO_TTY}" -ne 0 ]] || __getYesNoInput "${IRED}This will erase everything on these disks.${RST}\nAre you sure you want to continue?" "${IRED}No"; then
	...
	[[ "${_arguments}" != "${_arguments// --delete-networking }" ]] && {
		if [[ "${NO_TTY}" -ne 0 ]] || __getYesNoInput 'Are you sure you want to delete all network interface configuration files?' "${IRED}No"; then
	...
	if (( ${_fullDetails} )); then
		fold -sw $(tput cols) <<-EOF
			...
			-d, --debug                    Turns on debugging output
			    ...
			    --no-tty                   Enables non-interactive mode when running functional options
...
function ss__removeDiskMounts() {
	...
	(
		...
		if [[ ${CENTOS_VERSION} -eq "7" ]] && ! systemctl daemon-reload; then
			__logMessage "systemd daemon-reload failed after removing ${ss_DISK_MOUNTPATH}${_mountpoint} from ${FSTAB}"
		fi
		return 0
	)
...
function ss__resizePartition() {
	...
	if [[ -b "${_device}" && -b "${_device}${_partition}" ]]; then
		...
		__printMessage
		[[ "${NO_TTY}" -ne 0 ]] || __getYesNoInput 'Are you sure you want to continue?' 'Yes' || return 0
...
#ss__runOnCluster(Command, ClusterPassword="", HideCommand=${false}, singleNode="")
...
function ss__saveSetting() {
	...
	local _variableValue="${!1}"
	__printDebugMessage "_variableName=${_variableName}; _variableValue=${_variableValue}"

	set +e
	grep "${_variableName}" "${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}" >/dev/null
	grep_res="$?"
	set -e
	if [ $grep_res -eq 0 ]; then
		...
		sed -i "s~\(^declare\( -i\)\? ${_variableName}=['\"]\?\)[^'\"]*\(['\"]\?\)~\1${_variableValue}\3~" "${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}"
		__printDebugMessage "After sed replacement: $(grep -E "declare( -i)? ${_variableName}" "$(__getDirectoryName)/$(__getFileName)")" ${true} 2
	else
		_integer=""
		is_number='^[0-1]+$'
		if [[ "${_variableValue}" =~ ${is_number} ]]; then
			_integer="-i "
		fi
		echo "declare ${_integer}${_variableName}=${_variableValue}" >> "${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}"
	fi
...
function ss__setDomainName() {
	...
	if ! echo "${_currentHostname}" | grep -q '\..*'; then
		ss__createHostsFile
		if [[ -n "${_currentDomainname}" ]] && grep -E -i -v "^(127.0.0.1|::1)" "${HOSTS_FILE}" | grep -i -q "${_currentHostname}.${_currentDomainname}"; then
			__printMessage "Replacing old entries in ${HOSTS_FILE} ... " ${false}
			sed -i -r -e "/^(127.0.0.1|::1)/! s~${_currentHostname}.${_currentDomainname}~${_currentHostname,,}.${_domainName,,}~gI" "${HOSTS_FILE}"
		elif [[ "${_currentHostname,,}" != 'localhost' && "${_currentHostname,,}" != 'localhost.localdomain' ]]; then
			if grep -E -i -v "^(127.0.0.1|::1)" "${HOSTS_FILE}" | grep -i -q "${_currentHostname}"; then
				__printMessage "Updating existing entry in ${HOSTS_FILE} ... " ${false}
				sed -i -r -e "/^(127.0.0.1|::1)/! s~${_currentHostname}~${_currentHostname,,}.${_domainName,,}~gI" "${HOSTS_FILE}"
			else
...
function ss__setHostname() {
	...
	if [[ "${_hostname,,}" != 'localhost' && "${_hostname,,}" != 'localhost.localdomain' ]]; then
		if [[ -n "${_currentHostname:-}" ]] && [[ "${_currentHostname,,}" != 'localhost' && "${_currentHostname,,}" != 'localhost.localdomain' ]] && grep -i -q "${_currentHostname}" "${HOSTS_FILE}"; then
			__printMessage "Replacing old entries in ${HOSTS_FILE} ... " ${false}
            if echo "${_hostname}" | grep -q '\..*' && [[ -n "${_currentDomainname:-}" ]]; then
                sed -i "s~${_currentHostname}.${_currentDomainname}~${_hostname,,}~gI" "${HOSTS_FILE}"
			else
			...
			_ipAddress="$(hostname -I | awk '{print $1}')"
			if [[ -n "${_ipAddress:-}" ]]; then
		  		if echo "${_hostname}" | grep -q '\..*'; then
					printf '%s\t%s\t%s\n' "${_ipAddress}" "${_hostname,,}" "${_hostname%%.*}" >> "${HOSTS_FILE}"
				else
                    if [[ -n "${_currentDomainname:-}" ]]; then
	...
	__printMessage 'Updating kernel ... ' ${false}
	[[ ${CENTOS_VERSION} -eq "6" ]] && sysctl -q -w kernel.hostname="${_hostname,,}"
	[[ ${CENTOS_VERSION} -eq "7" ]] && hostnamectl set-hostname "${_hostname,,}"
	__printMessage "${IGREEN}Done"

	echo "${_hostname}" | grep -q '\..*' && ss__setDomainName "${_hostname#*.}"
...
function ss__menu_main() {
	...
	while (( ${true} )); do
		_menuOptions=(
			...
			'Prep New Node' #10
			'' #11
			'B=BMC Configuration' #12
			'' #13
			'R=Run Pre-installation Checks' #14
			'' #15
			'D=Download HyperStore Files' #16
			"**${IRED}Please Download or place the HyperStore files in '${ss_STAGING_DIRECTORY}'" #17
			'' #18
			'S=Script Settings' #19
			"**${IRED}Staging Directory '${ss_STAGING_DIRECTORY}' Not Found" #ss__hyperstoreSysPrepDownloader 20
			"A=About $(__getFileName)" #21
			'' #22
			"X=${IRED}Exit" #23
		)

		if [[ ! -f "$(__getDirectoryName)/${PIC_FILE}" && ! -f "${ss_STAGING_DIRECTORY}/${PIC_FILE}"  && ! -f "$(which ${PIC_FILE} 2>/dev/null)" ]]; then #hide pre-installation checks options when script isn't found
		...
		[[ -d "${ss_STAGING_DIRECTORY}" ]] && _menuOptions[20]="--${_menuOptions[20]}" #hide staging directory warning when directory exists
		...
		if [[ ! -f "${_hyperstoreFile:-}" ]]; then #hide entries that need the binary file
			...
			_menuOptions[15]="--${_menuOptions[15]}"
			_menuOptions[16]="--${_menuOptions[16]}"
			_menuOptions[17]="--${_menuOptions[17]}"
		fi
		ss__isCurrentPasswordSimple || _menuOptions[6]="--${_menuOptions[6]}"

		if ! dmidecode 2>/dev/null | grep -A 6 -i ipmi | grep "Base Address:" | grep -v "0x(0)* " > /dev/null 2>&1; then
		...
		if __getMenuInput 'Choice:' '_menuOptions' '_selection'; then
			case "${_selection}" in
				...
				'5') #Change Password
					while (( ${true} )); do
							local _changePasswordOptions=(
								"Change ${USER} Password"
								''
								"P=${IYELLOW}Return to the Previous Menu"
							)
							local _changePasswordSelection
							__printTitle 'Run On Cluster'
							if __getMenuInput 'Choice:' '_changePasswordOptions' '_changePasswordSelection'; then
								case "${_changePasswordSelection}" in
									'1')
				...
				'6') #Configure HyperStore Prerequisites
					while (( ${true} )); do
						local _runPrereqMenuOptions=(
							'Install & Configure Prerequisites'
							''
							"P=${IYELLOW}Return to the Previous Menu"
						)
						local _runPrereqSelection
						__printTitle 'Run On Cluster'
						if __getMenuInput 'Choice:' '_runPrereqMenuOptions' '_runPrereqSelection'; then
							case "${_runPrereqSelection}" in
								'1')
									__printTitle 'Install & Configure Prerequisites'
									__printMessage
									if grep -q -E "${ss_SURVEY_ENABLED_REGEX}" "$(ss__getSurveyFileName)" 2>/dev/null && __getYesNoInput 'Would you like to perform this on all nodes listed in your survey file?' 'Yes'; then
										...
										[[ ! -f "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" ]] && ss__extractHyperStoreBinaryPackagedFile "${SELFEXTRACT}" || :
										[[ -f "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" ]] && __printMessage "${IWHITE}Copying Packages to cluster nodes" && ss__copyFileToCluster "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" "${_clusterPassword:-}"
										...
										__printMessage "${IWHITE}Copying configuration file to cluster nodes"

										ss__copyFileToCluster "$PRE_INSTALL_CONF_DIR/$PRE_INSTALL_CONFIG" "${_clusterPassword:-}"
				...
				'7') #Run command across cluster
					while (( ${true} )); do
						local _runCMDMenuOptions=(
							'Enter Command to Run on Cluster'
							''
							"P=${IYELLOW}Return to the Previous Menu"
						)
						local _runCMDSelection
						__printTitle 'Run On Cluster'
						if __getMenuInput 'Choice:' '_runCMDMenuOptions' '_runCMDSelection'; then
							case "${_runCMDSelection}" in
								'1')
									if [[ -f "$(ss__getSSHKeyFileName)" ]]; then
										__printMessage "Using SSH Key: $(ss__getSSHKeyFileName)"
										__printMessage 'If you are prompted for your password, try "Push SSH Key File to cluster nodes" in Script Settings'
				...
				'8') #Copy file to cluster nodes
					while (( ${true} )); do
						local _clusterFileOptions=(
							'Copy File to Cluster Nodes'
							''
							"P=${IYELLOW}Return to the Previous Menu"
						)
						local _clusterFileSelection
						__printTitle 'Run On Cluster'
						if __getMenuInput 'Choice:' '_clusterFileOptions' '_clusterFileSelection'; then
							case "${_clusterFileSelection}" in
								'1')
									while :; do
				...
				'9') #Add node prep
					while (( ${true} )); do
						local _addNodePrepOptions=(
							'Prep New Node'
							''
							"P=${IYELLOW}Return to the Previous Menu"
						)
						local _addNodePrepSelection
						__printTitle 'Run On Cluster'
						if __getMenuInput 'Choice:' '_addNodePrepOptions' '_addNodePrepSelection'; then
							case "${_addNodePrepSelection}" in
								'1')
									__clearScreen
									__printTitle "Prep New Node"
									local _newNodeIpAddress
									__getIPAddressInput 'IP Address of new node:' '_newNodeIpAddress' || continue
									while ! ping -c 1 -W 3 "${_newNodeIpAddress}" > /dev/null 2>&1; do
										__printMessage "The IP address '${_newNodeIpAddress}' can't be reached, please try a different one!\n"
										__getIPAddressInput 'IP Address of new node:' '_newNodeIpAddress' || continue 2
									done

									__printMessage
									echo "${IWHITE}Attempting to install SSH key on ${_newNodeIpAddress}"
									ss__installSSHKeyFile "$(ss__getSSHKeyFileName)" "${_clusterPassword:-}" "${_newNodeIpAddress}"

									echo "${IWHITE}Attempting to copy self extract installer to ${_newNodeIpAddress}"
									ss__copyFileToCluster "${ss_STAGING_DIRECTORY}/${SELFEXTRACT}" "" "${_newNodeIpAddress}"

									echo "${IWHITE}Attempting to copy System Setup script to ${_newNodeIpAddress}"
									ss__copyFileToCluster "$(__getDirectoryName)/$(__getFileName)" "" "${_newNodeIpAddress}"

									ss__runOnCluster "$(__getDirectoryName)/$(__getFileName) --remote --prepare $(__isDebugEnabled && printf '%s' ' --debug')" "${_clusterPassword:-}" ${true} "${_newNodeIpAddress}";
				...
				'A') #About Information
					...
					__pause
					;;
				'B') ss_menu_bmc_config ;; #BMC Configuration
				'D') #Download Cloudian HyperStore Binary File (GA)
					local -a _downloadMenuOptions
					local _downloadSelection
					while (( ${true} )); do
						...
						if __downloadURI "${ss_versionsURL}" "$(__getDirectoryName)/versions.txt" 'HyperStore Version Information'; then
							__printMessage
							_downloadMenuOptions=(
								'Download HyperStore GA files' #1
								'Download HyperStore EA files' #2
								''
								"P=${IYELLOW}Return to the Previous Menu"
							)
							if __getMenuInput 'Choice:' '_downloadMenuOptions' '_downloadSelection'; then
								case "${_downloadSelection}" in
									'1') ss__hyperstoreSysPrepDownloader "$(__getDirectoryName)/versions.txt" "HyperStore-GA" ;;
									'2') ss__hyperstoreSysPrepDownloader "$(__getDirectoryName)/versions.txt" "HyperStore-EA" ;;
									'P') break;; #Return to the previous menu
								esac
							fi
					...
					;;
				'R') ss__open_pre_install_menu ;; #Pre-Installation Checks
...
#ss__menu_prepare()
	#Simplified main menu for adding a node
function ss__menu_prepare() {
	trace "${FUNCNAME[0]}" "$@"
	...
	while (( ${true} )); do
		_menuOptions=(
			...
			'Script Settings' #18
			''
			"X=${IRED}Return to Master Node" #22
		)
		...
		if __getMenuInput 'Choice:' '_menuOptions' '_selection'; then
			case "${_selection}" in
				...
				'3')
					...
					;;
				'4') ss__menu_disks ;; #Configure Disks
				'5') ss__menu_settings ;; #Settings
				'X') exit 0 ;; #Exit
			esac
		else
			break
...
function ss__open_pre_install_menu() {
	trace "${FUNCNAME[0]}" "$@"

	[[ ${#} -ne 0 ]] && __raiseWrongParametersError ${#} 0
	local _pic_file=./${PIC_FILE}

	if [ -f "$(__getDirectoryName)/${PIC_FILE}" ]; then
		_pic_file="$(__getDirectoryName)/${PIC_FILE}"
	elif [ -f "${ss_STAGING_DIRECTORY}/${PIC_FILE}" ]; then
		_pic_file="${ss_STAGING_DIRECTORY}/${PIC_FILE}"
	fi
	#run preinstall check with the "always exit 0" param
	$_pic_file -y
}
...
function ss__menu_networking() {
	...
	#clear search and domain entries from /etc/resolv.conf if domain is unset
	if ! __getDomainName >/dev/null 2>&1; then
		sed -i -E '/^(search|domain)/d' "${RESOLV_CONF}"
		__logMessage "Cleared 'domain' and 'search' entries from ${RESOLV_CONF}"
	fi
	...
	while (( ${true} )); do
		...
		_menuOptions+=(
			...
			"$((( ss_RESTART_NETWORKING )) || printf '%s' '--')++${IRED}Network settings have changed!${RST}"
...
function ss__menu_settings() {
	...
	while (( ${true} )); do
		_menuOptions=(
			...
			'__Disk Settings'
			"${IPURPLE}lazy_init and noinit_itable: ${RST}:  $((( ss_LAZYINIT_MOUNTNOINIT )) && printf "${IGREEN}Enabled" || printf "${IRED}Disabled")"
			''
			"P=${IYELLOW}Return to the Previous Menu"
		...
		case "${_selection}" in
			'1') #Toggle Use Colors
				...
				ss__saveSetting 'ss_USE_COLORS'
				;;
			'2') __toggleBoolean 'PRINT_MENU_COMMENTS' ;; #Toggle Menu Help
			'3') __toggleBoolean 'DEBUG' ;; #Toggle Debug
			'4') #Configure Input Attempts
			...
			'5') #Max Column Width
				...
				;;
			'6') #Max Items for single column menu
				...
				;;
			'7') ss__changeStagingDirectory ;; #Change Staging Directory
			'8') ss__changeSurveyFileLocation ;; #Change Survey File Location
			'9') ss__changeSSHKeyFileLocation ;; #Change SSH Key File Location
			...
			'13') #Change VLAN Naming Style
				...
				;;
			'14') #Toggle Lazy Init/Mount No Init
				__toggleBoolean 'ss_LAZYINIT_MOUNTNOINIT'
				ss__saveSetting 'ss_LAZYINIT_MOUNTNOINIT'
				;;
			'P') break ;; #Previous Menu
...
function ss_menu_bmc_config() {
	...
	if ! dmidecode 2>/dev/null | grep -A 6 -i ipmi | grep "Base Address:" | grep -v "0x(0)* " > /dev/null 2>&1; then
		echo "This machine has no ipmi device configured!"
		return 0
	elif ! ipmitool -h > /dev/null 2>&1; then
		echo "ipmitool is not installed! Can't continue with BMC configuration!"
		return 0
	fi
	while (( ${true} )); do
		__clearScreen
		local config=$(ipmitool lan print)
		local addressMode=$(echo "${config}" | grep 'IP Address Source' | sed 's/^.*: //' | sed 's/ Address//')
		local IPAddress=$(echo "${config}" | grep 'IP Address  ' | sed 's/^.*: //')
		local netMask=$(echo "${config}" | grep 'Subnet Mask' | sed 's/^.*: //')
		local defaultGateway=$(echo "${config}" | grep 'Default Gateway IP' | sed 's/^.*: //')
		_menuOptions=(
			"Print BMC LAN configuration"
			"Toggle IP source ($addressMode)"
			"Change IP address ($IPAddress)"
			"Change Subnet Mask ($netMask)"
			"Change Default Gateway Address ($defaultGateway)"
			"Change ipmi user passwords"
			''
			"P=${IYELLOW}Return to the Previous Menu"
		)

		__printTitle "BMC Configuration"
		if __getMenuInput 'Choice:' '_menuOptions' '_selection'; then
			case "${_selection}" in
				'1')
					__clearScreen
					__printTitle "BMC Configuration" && ipmitool lan print
					__printMessage && __pause

					;;
				'2')
					__clearScreen
					__printTitle "BMC Configuration"
					if echo "${addressMode}" | grep -q 'DHCP'; then
						__printMessage "Setting IP source to static ... " "${false}"
						_cursorPosition=($(__getCursorPosition)); __printMessage
						if ipmitool lan set 1 ipsrc static > /dev/null 2>&1; then
							__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
						else
							__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
						fi
					else
						__printMessage "Setting IP source to DHCP ... " "${false}"
						_cursorPosition=($(__getCursorPosition)); __printMessage
						if ipmitool lan set 1 ipsrc dhcp > /dev/null 2>&1; then
							__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
						else
							__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
						fi
				...
				'3')
					__clearScreen
					__printTitle "BMC IP address"
					local _ipAddress
					__getIPAddressInput 'IP Address:' '_ipAddress' "${IPAddress:-0.0.0.0}" || continue
					__printMessage "Setting IP address for ipmi channel 1 to ${IWHITE}${_ipAddress}${RST} ... " "${false}"
					_cursorPosition=($(__getCursorPosition)); __printMessage
					if ipmitool lan set 1 ipaddr "${_ipAddress}" > /dev/null 2>&1; then
						__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
					else
						__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
					fi
				...
				'4')
					__clearScreen
					__printTitle "BMC Subnet Mask"
					local _ipAddress
					__getIPAddressInput 'Subnet Mask:' '_ipAddress' "${netMask:-0.0.0.0}" || continue
					__printMessage "Setting subnet mask for ipmi channel 1 to ${IWHITE}${_ipAddress}${RST} ... " "${false}"
					_cursorPosition=($(__getCursorPosition)); __printMessage
					if ipmitool lan set 1 netmask "${_ipAddress}" > /dev/null 2>&1; then
						__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
					else
						__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
					fi
				...
				'5')
					__clearScreen
					__printTitle "BMC Default Gateway"
					local _ipAddress
					__getIPAddressInput 'Default Gateway:' '_ipAddress' "${IPAddress:-0.0.0.0}" || continue
					__printMessage "Setting default gateway for ipmi channel 1 to ${IWHITE}${_ipAddress}${RST} ... " "${false}"
					_cursorPosition=($(__getCursorPosition)); __printMessage
					if ipmitool lan set 1 defgw ipaddr "${_ipAddress}" > /dev/null 2>&1; then
						__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
					else
						__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
					fi
				...
				'6')
					while (( ${true} )); do
						__clearScreen
						__printTitle "Change BMC User Passwords"
						local -a userMenuOptions
						local userSelection
						IFS=$'\n'; userMenuOptions=($(ipmitool user list 1)); unset IFS
						userMenuOptions[0]="++ID  Name	           Callin  Link Auth  IPMI Msg   Channel Priv Limit"
						if [[ "${#userMenuOptions[@]}" -gt 1 ]]; then
							for idx in $(seq 1 $((${#userMenuOptions[@]}-1))); do
								userMenuOptions[$idx]="$(echo ${userMenuOptions[$idx]} | awk '{print $1}')=${userMenuOptions[$idx]}"
								if echo ${userMenuOptions[$idx]} | grep -q -E "NO ACCESS|Unknown"; then
									userMenuOptions[$idx]="--${userMenuOptions[$idx]}"
								fi
							done
						else
							echo "No users found!"
						...
						__getMenuInput 'Choice:' 'userMenuOptions' 'userSelection' || break
						case "${userSelection}" in
							'P') #Return to the previous menu
								break;;
							*)
								__clearScreen
								__printTitle "Change Password"
								local _password
								__getInput 'New password:' '_password' '' ${true} || break
								__printMessage "Changing password for user '${userSelection}' ... " "${false}"
								_cursorPosition=($(__getCursorPosition)); __printMessage
								if ipmitool user set password "${userSelection}" "${_password}" > /dev/null 2>&1; then
									__printStatusMessage ${_cursorPosition[*]} "${IGREEN}Done"
								else
									__printStatusMessage ${_cursorPosition[*]} "${IRED}Failed"
								fi
...
if ! __isSourced; then
	if [ ! -f "${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}" ]; then
		__logMessage "Configuration file, ${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}, is missing. Creating empty file."
  	ss__createConfigFile "${PRE_INSTALL_CONF_DIR}" "${PRE_INSTALL_CONFIG}"
	fi
	source "${PRE_INSTALL_CONF_DIR}/${PRE_INSTALL_CONFIG}"
